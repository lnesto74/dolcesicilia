...
function dayLabelSg(dateKey) {
  return parseOrderDate(`${dateKey}T12:00:00Z`).toLocaleDateString('en-SG', {
    timeZone: SG_TZ,
    weekday: 'short',
    day: 'numeric',
    month: 'short',
...
function buildOrderHeatmap(orders, repeatOnly = false) {
  const grid = new Map();
...
function fillOrdersByDay(byDay, byRevenue) {
...
function buildRepeatTimelines(orders) {
  ...
  return timelines.sort(
    (a, b) => b.orderCount - a.orderCount || (b.avgDaysBetween ?? 0) - (a.avgDaysBetween ?? 0),
  );
}

export function computeOrderAnalytics(orders, contacts = []) {
  const now = new Date();
  ...
  const ordersThisWeek = orders.filter((o) => parseOrderDate(o.ordered_at) >= weekAgo).length;
  const ordersThisMonth = orders.filter((o) => parseOrderDate(o.ordered_at) >= monthAgo).length;

  const byHour = Array.from({ length: 24 }, (_, h) => ({ hour: h, count: 0 }));
  const byDay = new Map();
  const byRevenue = new Map();
  const byWeekday = new Map(WEEKDAY_ORDER.map((day) => [day, 0]));
  ...
  return {
    ...
    ordersByDay,
    ordersByWeekday: WEEKDAY_ORDER.map((day) => ({
      day,
