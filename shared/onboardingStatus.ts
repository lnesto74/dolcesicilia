...
export type WelcomeStatus = 'not_sent' | 'queued' | 'sent_awaiting' | 'done';

export interface OnboardingOptInView {
  welcomeStatus: WelcomeStatus;
  welcomeLabel: string;
  welcomeShortLabel: string;
  welcomeChipClass: string;
  pref: MessagePref;
  prefLabel: string | null;
  prefShortLabel: string | null;
  prefChipClass: string;
  summary: string;
  welcomeSentAt?: string;
  prefUpdatedAt?: string;
}

const WELCOME_CHIP: Record<
  WelcomeStatus,
...
const PREF_CHIP: Record<MessagePref, string> = {
...
export function prefDisplayLabel(pref: MessagePref, endOfDayOptin?: boolean): string | null {
...
export function prefShortDisplayLabel(pref: MessagePref, endOfDayOptin?: boolean): string | null {
  if (pref === 'unset') return null;
...
export function computeOnboardingOptInView(contact: {
  ...
  end_of_day_optin?: boolean | null;
  welcomeSent?: boolean;
  welcomeSentAt?: string | null;
  onboardingScheduleStatus?: string | null;
  followup_status?: string | null;
  sentMessages?: { template_id: string; sent_at?: string }[] | null;
}): OnboardingOptInView {
  const pref = normalizeMessagePref(contact.message_pref) as MessagePref;
  ...
  let welcomeStatus: WelcomeStatus = 'not_sent';
  if (pref !== 'unset') {
    welcomeStatus = 'done';
  } else if (welcomeSent) {
...
export function hasWelcomeMessageSent(
  sentMessages?: { template_id: string; sent_at?: string }[] | null,
): boolean {
  return (
    sentMessages?.some((m) => m.template_id === PREFERENCE_POLL_TEMPLATE_ID) ?? false
  );
}

export function welcomeSentAtFromMessages(
  sentMessages?: { template_id: string; sent_at?: string }[] | null,
): string | null {
