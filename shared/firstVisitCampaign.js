...
export function formatNumberedQuestion(header, options) {
  return [header, ...options.map((o, i) => `${OPTION_EMOJIS[i]} ${o}`)].join('\n');
}

export const FIRST_VISIT_MESSAGES = [
  {
    step: 'ready_msg1',
    nextStep: 'msg1_sent',
...
export function getMessageForStep(step) {
...
export function getPollOptionsForWaiting(waitingFor) {
  if (!waitingFor) return null;
  const step = WAITING_POLL_STEP[waitingFor];
  return step ? getPollOptions(step) : null;
}

export function answerLabelFor(replyKey, value) {
...
export function parseCampaignReply(body, waitingFor, pollOptions) {
...
export function formatStoredAnswer(raw) {
  if (!raw) return null;
...
export function fillCampaignMessage(body, customerName, senderName = 'Luca') {
