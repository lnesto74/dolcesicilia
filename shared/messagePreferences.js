...
/** @typedef {'every_launch' | 'monthly' | 'launches_off' | 'opt_out' | 'weekly' | 'unset'} MessagePref */

export const PREFERENCE_POLL_OPTIONS = [
  {
    rowId: 'pref_everything',
    number: 1,
    messagePref: 'every_launch',
    endOfDayOptin: true,
...
export const MESSAGE_PREF_LABELS = {
...
export function normalizeMessagePref(value) {
  if (
    value === 'every_launch' ||
    value === 'weekly' ||
    value === 'monthly' ||
...
/**
 * @returns {{ messagePref: MessagePref, endOfDayOptin: boolean, confirmation: string, rowId: string } | null}
 */
export function parsePreferenceReply(rawBody, extra = {}) {
  ...
  if (/\b(end.of.day|evening tray|kinder price|🌙)\b/i.test(body)) {
    const opt = PREFERENCE_POLL_OPTIONS.find((o) => o.rowId === 'pref_end_of_day');
    return {
      messagePref: opt.messagePref,
      endOfDayOptin: opt.endOfDayOptin,
      confirmation: opt.confirmation,
  ...
  if (/\b(now and then|monthly|occasional|📅)\b/i.test(body)) {
    const opt = PREFERENCE_POLL_OPTIONS.find((o) => o.rowId === 'pref_monthly');
    return {
      messagePref: opt.messagePref,
      endOfDayOptin: opt.endOfDayOptin,
      confirmation: opt.confirmation,
  ...
  if (/\b(everything|new dolci|evening trays|🌿)\b/i.test(body)) {
    const opt = PREFERENCE_POLL_OPTIONS.find((o) => o.rowId === 'pref_everything');
    return {
      messagePref: opt.messagePref,
      endOfDayOptin: opt.endOfDayOptin,
      confirmation: opt.confirmation,
  ...
  if (/\b(launch|every|whenever|something new|🍋)\b/i.test(body)) {
    const opt = PREFERENCE_POLL_OPTIONS.find((o) => o.rowId === 'pref_every_launch');
    return {
      messagePref: opt.messagePref,
      endOfDayOptin: opt.endOfDayOptin,
      confirmation: opt.confirmation,
...
export function fillPreferenceIntro(body, name) {
...
export const MESSAGE_PREF_CHIP_CLASS = {
