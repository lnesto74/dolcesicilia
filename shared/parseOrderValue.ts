...
export type GrabScreenshotType = 'order_detail' | 'edit_call' | 'unknown';

export interface OrderValueResult {
  orderValue: number;
  currency: 'SGD';
  source: 'total' | 'subtotal' | 'items' | 'edit_cap';
  raw: string;
  isNewCustomer?: boolean;
  screenType: GrabScreenshotType;
}
...
export function detectGrabScreenshotType(text: string): GrabScreenshotType {
...
function findMoneyAfterLabel(text: string, label: string): number | null {
...
/** cap = order_value × (1 + pct) → order_value = cap / (1 + pct) */
function valueFromEditCap(text: string): OrderValueResult | null {
  const capPatterns = [
...
function sumLineItems(text: string): number | null {
...
/** Extract order total from Grab screenshot OCR text. */
export function extractOrderValue(text: string): OrderValueResult | null {
...
export function isHighValueOrder(value: number | null | undefined): boolean {
